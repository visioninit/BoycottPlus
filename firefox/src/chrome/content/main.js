...
var boycottplus = {
	...
	,onPageLoad: function() {
		...
		if ( boycottplus.prefs.getBoolPref("just_installed") ) {
			...
			window.openDialog(
				...
				'just_installed',
				'modal,centerscreen,resizable,chrome,titlebar,resizable=no'
			);
		} else {	
	...
	*/
	...
	,doSubscribe: function() {
		...
		// If already subscribed, remove source.
        try {
			boycottPlus.data.removeSource(source, false);
		}
		catch(e) {
	...
	,checkSitePrefs: function(tabBrowser, url, completeAddress) {
		// if there already is a bar, remove it
		var browser = tabBrowser;
        ...
        if (company) {
			...
			// Don't Boycott Site if it's allowed.
			var exitAllowed = false;
			boycottPlus.allowed.forEach(function(allowed) {
				if ( url == allowed ) {
					exitAllowed = true;
					return;
				}
			...
			if( company[0]._boycott.notificationType == '1' ) {
				boycottPlus.bar._addBar(notification.ownerDocument, notification, company, gBrowser, url);
			} else if( company[0]._boycott.notificationType == '2' ) {
				boycottplus.blockSite(browser, url, completeAddress, company[0]);
			}
        }
	}
	
	// Periodically check for Campaign update depending on updateFrecuency.
	...
	// Add the url to the allowed array.
	,addAllowed: function(url) {
		boycottPlus.allowed.push(url);
	}

	// Block the Banned Site.
	,blockSite: function(browser, url, completeAddress, company) {
		...
		window.stop();
		/*
		if ( browser.userTypedValue != url ) {
			window.loadURI(browser._userTypedValue);
		}
		*/
		var docHtml = browser.contentDocument;
